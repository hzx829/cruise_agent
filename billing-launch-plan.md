...
这样用户看到的是月度额度点数，而不是“保证能完整跑 N 次 `glm-5` agent”。产品页面不再把额度写成“次数”。
...
| 月包 | 价格 | 额度点数 | 点数单价 | 适合场景 |
| --- | ---: | ---: | ---: | --- |
| 轻量月包 | ¥29/月 | 600 | ¥0.048 | 偶尔查价、保存方案 |
| 标准月包 | ¥89/月 | 3000 | ¥0.030 | 持续比价、跟进降价 |
| 专业月包 | ¥199/月 | 8000 | ¥0.025 | 高频选品、多目的地对比 |

折算为当前 `glm-5` agent 的大致可用量：

| 月包 | 私有库报价，约 12 点/次 | 含 1 次网络搜索，约 17 点/次 | 含 2 次网络搜索，约 22 点/次 |
| --- | ---: | ---: | ---: |
| 轻量月包 | 50 次 | 35 次 | 27 次 |
| 标准月包 | 250 次 | 176 次 | 136 次 |
| 专业月包 | 666 次 | 470 次 | 363 次 |
...
- 当前“月包”语义是购买后立即发放额度，有效期 1 个月；当前版本不自动续费。
- 当前版本不做自动续费，按每笔购买的付款时间计算 1 个月有效期。
...
- `billing_plans`：可售月度额度包。
- `billing_orders`：本地订单和支付宝交易号。
- `payment_events`：支付宝通知、查询、手工事件。
- `credit_ledger`：额度加减流水。
- `credit_grants`：月包权益桶，记录每笔权益的剩余额度和过期时间。

余额从未过期的 `credit_grants.remaining` 汇总。月包支付成功加一个有效期 1 个月的权益桶；一次成功 agent run 按实际 token 和网络搜索成本，从最早过期的权益桶开始扣点。
