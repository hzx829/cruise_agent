...
function buildCsv(runs: AgentRunWithSteps[]): string {
  ...
  const rows = runs.map((run) => {
    ...
    return [
      ...
      run.user_query,
      run.duration_ms,
      run.timingSummary.observedDurationMs,
      run.timingSummary.stepDurationMs,
      run.timingSummary.modelDurationMs,
      run.timingSummary.toolWallTimeMs,
      run.timingSummary.toolDurationMs,
      run.timingSummary.unattributedDurationMs,
      run.finish_reason,
      run.assistant_text_len,
      run.total_tokens,
      run.prompt_version,
      run.stepCount,
      run.timingSummary.stepTimingCount,
      run.toolNames,
