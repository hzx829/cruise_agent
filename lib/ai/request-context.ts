...
export function formatRequestContextForPrompt(
  ...
  return `## 用户背景
- 用户网络出口位置: ${formatLocation(context.location)}

使用方式:
- 这是由 IP/CDN 推断出的网络出口位置，可能是运营商出口或代理节点，不等于用户真实所在地。
- 不要默认用户就在该城市。回答涉及附近出发港、机场、接驳、本地可达性时，如果用户没有明确地点，先询问用户从哪个城市或省份出发，或给主要出发地的通用方案。
- 如果用户在问题或历史对话里明确给出城市、省份、港口、国家或地区，以用户明确给出的地点为准，并覆盖网络出口位置。
- 不要声称精确定位，也不要提及 IP 地址。`;
}
